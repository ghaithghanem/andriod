...
class SQLiteHelper(context: Context):SQLiteOpenHelper(context,DATABASE_NAME,null,DATABASE_VERSION){
    ...
    override fun onCreate(p0: SQLiteDatabase?) {
        val creatTblStudent =("CREATE TABLE " + TBL_STUDENT +"("
                + ID + " INTEGER PRIMARY KEY," + NAME + " TEXT,"
                + EMAIL +" TEXT" + ")")
        p0?.execSQL(creatTblStudent)
    }
    ...
    fun updateStudent(std: StudentModel): Int{
        ...
        contentValues.put(NAME, std.name)
        contentValues.put(EMAIL, std.email)
        val success = p0.update(TBL_STUDENT, contentValues,"id="+std.id,null)
        p0.close()
        return success
    }
    fun deleteStudentById(id: Int): Int {
        ...
        val success = p0.delete(TBL_STUDENT,"id=$id",null)
        p0.close()
        return success
    }
}